...
# Best-effort: se la build vive sull'immagine APFS dedicata e non è montata,
# montala. Su altre macchine i path non esistono → blocco no-op.
if [ ! -d "/Volumes/wzbuild/build" ] && [ -f "/Volumes/HD 1000 Gb/wzbuild.sparseimage" ]; then
  hdiutil attach "/Volumes/HD 1000 Gb/wzbuild.sparseimage" >/dev/null 2>&1 || true
fi

if [ -n "${WZ_BUILD_DIR:-}" ]; then
  WZ_BUILD="$WZ_BUILD_DIR"
elif [ -d "/Volumes/wzbuild/build" ]; then
