...
[[maybe_unused]] static void moveToParentRightEdge(WIDGET *widget, int32_t right)
{
...
void startTitleMenu()
{
	...
	addBackdrop();
	addTopForm(false);
	addBottomForm(false, true); // DedrisRemastered: pannello trasparente nel menu principale
	...
	// DedrisRemastered: rimossi i link del menu principale (sito ufficiale, dona,
	// chat Discord/IRC e controllo versione online) per un look pulito.
}
...
void addBottomForm(bool wide, bool transparent)
{
	...
	{
		botForm = std::make_shared<IntFormAnimated>();
	}
